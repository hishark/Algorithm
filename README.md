...
|PATA1047|数据范围很大的情况一般最好用char数组来存放数据，用string很容易超时</br>这一题用cincout也会超时|
|PATA0163|统计数量的时候，很多情况下可以置初值为max，然后碰到不符合条件的就减一</br>printf("%.1f%")可输出百分比，如果要紧跟一个百分号的话再自己加一个%</br>st.find(value)!=st.end()为true就表示在st中找到了value|
